...
export default function HorizontalLinearStepper() {
  ...
  const handleSkip = () => {
    if (!isStepOptional(activeStep)) {
      ...
      throw new Error("نمی توانید از مرحله ای که اختیاری نیست بگذرید.");
    }
    ...
    <Box sx={{ width: '100%' }}>
      ...
      {activeStep === steps.length ? (
        <React.Fragment>
          ...
          تمام مراحل انجام شد - شما کار را به اتمام رسانده اید.
          </Typography>
          <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
            <Box sx={{ flex: '1 1 auto' }} />
            <Button onClick={handleReset}>بازنشانی کنید</Button>
          </Box>
        </React.Fragment>
      ) : (
        <React.Fragment>
