  ...
  return (
    <div className={clsx('w-full', className)}>
      ...
      <Typography className="flex items-center space-x-6 text-13" color="text.secondary">
        ...
        <span className="whitespace-nowrap leading-none">{`${course.duration} دقیقه`}</span>
      </Typography>
      <Typography className="flex items-center space-x-6 text-13 mt-8" color="text.secondary">
        ...
        <span className="whitespace-nowrap leading-none">
          {course.progress.completed === 1 && '2 قسمت مشاهده شده'}
          {course.progress.completed === 2 && '1 قسمت مشاهده شده'}
          {course.progress.completed > 2 && `${course.progress.completed} قسمت مشاهده شده`}
          {course.progress.completed <= 0 && 'مشاهده نشده'}
        </span>
      </Typography>
    </div>
  );
