...
function SimpleFormExample() {
    ...
    onSubmit={handleSubmit((_data) => console.info(_data))}>
				...
				<div className="mt-48 mb-16">
					...
					<Controller name="Autocomplete" control={control} defaultValue={[]} render={({ field: { onChange, value, onBlur, ref } }) => (<Autocomplete className="mt-8 mb-16" multiple freeSolo options={options} value={value} onChange={(_event, newValue) => {
                onChange(newValue);
				...
				<div className="mt-48 mb-16">
					...
					<Controller name="DateTimePicker" control={control} render={({ field: { onChange, value } }) => (<DateTimePicker value={new Date(value)} onChange={(val) => {
